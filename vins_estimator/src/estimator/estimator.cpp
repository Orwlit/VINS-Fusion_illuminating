    ...
    if (solver_flag == INITIAL)  //进入初始化步骤 tzhang
    {
        ...
        if (!STEREO && USE_IMU)  //单目+IMU版本的初始化
        {
            if (frame_count == WINDOW_SIZE)  //滑窗满后才开始初始化 tzhang
            {
                ...
                {
                    result = Estimator::initialStructure();  //对应VINS_mono论文 V章节
...
void Estimator::optimization()
{
    ...
    /*######优化参数：q、p；v、Ba、Bg#######*/
    for (int i = 0; i < frame_count + 1; i++)  //IMU存在时，frame_count等于WINDOW_SIZE才会调用Estimator::optimization() tzhang
    {
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization(); //自定义的SE3右扰动模型
        //TODO: SIZE_POSE为什么是7维（3维t、4维q），而不是6维（3维t、3维q）？
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)  //imu与camera外参
    {
        ...
        if ((ESTIMATE_EXTRINSIC && frame_count == WINDOW_SIZE && Vs[0].norm() > 0.2) || openExEstimation) //看不懂条件???
        {
        ...
        else
        {
            ...
            problem.SetParameterBlockConstant(para_Ex_Pose[i]); //上面if条件不满足的话，就不优化imu与camera外参
        }
    }
    ...
    if(USE_IMU)
    {
        for (int i = 0; i < frame_count; i++)  //预积分残差，总数目为frame_count
        {
            ...
            problem.AddResidualBlock(imu_factor, NULL,
                                     para_Pose[i],  //i时刻速度 7维
                                     para_SpeedBias[i], //i时刻偏置 9维
                                     para_Pose[j],      //j时刻速度 7维
                                     para_SpeedBias[j]);//j时刻偏置 9维
        }
    }
